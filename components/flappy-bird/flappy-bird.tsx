    ...
    const interval = setInterval(() => {
      setStars((prevStars) =>
        prevStars
          ...
          .map((star) =>
            star.y > 512 ? { ...star, y: 0, x: Math.random() * 288 } : star
          )
      );
    }, 16);
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      ...
      if (backgroundImage.current) {
        ...
        gradient.addColorStop(0, "#0f172a"); // Azul oscuro arriba
        gradient.addColorStop(1, "#1e3a8a"); // Azul más claro abajo
      ...
      pipes.forEach((pipe) => {
        // Pipe top
        ctx.fillStyle = "rgba(14, 165, 233, 0.7)"; // Color azul semitransparente
        ...
        ctx.lineWidth = 1;
        for (
          let i = pipe.topHeight + PIPE_GAP + 5;
          i < canvas.height;
          i += 10
        ) {
          ctx.beginPath();
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center mb-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-slate-900/40 border border-blue-500/30 rounded-lg p-4 backdrop-blur-sm">
            <div className="text-xl font-bold text-blue-200 tracking-wide">
              SCORE: {score}
            </div>
            <div className="text-lg text-blue-300 tracking-wide">
              HIGH SCORE: {highScore}
            </div>

            {!gameStarted && !gameOver && (
              <div className="mt-4 text-xl font-medium text-blue-300 tracking-wider">
                PRESS START TO PLAY
              </div>
            )}
          ...
          <div className="bg-slate-900/40 border border-blue-500/30 rounded-lg p-4 backdrop-blur-sm">
            <h3 className="text-lg font-medium text-blue-200 mb-3 tracking-wider">
              HOW TO PLAY
            </h3>
